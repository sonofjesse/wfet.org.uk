...
/**
 * All Insights block results (grid, empty state, pagination).
 *
 * @package SOJ_Core_Modern
 *
 * @var array $args {
 *     @type WP_Query $query            Insights query.
...
if (!$is_preview) {
    $pagination_html = function_exists('soj_all_insights_pagination_html')
        ? soj_all_insights_pagination_html($query, $paged, $context_post_id)
        : '';
}
?>

<?php if ($query->have_posts()) : ?>
    <div class="all-insights__grid" data-gsap-animate="stagger">
        <?php
    ...
    <?php if ($pagination_html) : ?>
        <nav class="all-insights__pagination" aria-label="<?php esc_attr_e('Insights pages', 'soj-core'); ?>">
            <p class="all-insights__pagination-label"><?php esc_html_e('Pages', 'soj-core'); ?></p>
            <div class="all-insights__pagination-links">
                <?php echo $pagination_html; ?>
            </div>
        </nav>
    <?php endif; ?>
<?php else : ?>
    <p class="all-insights__empty"><?php esc_html_e('No insights available yet.', 'soj-core'); ?></p>
<?php endif; ?>
