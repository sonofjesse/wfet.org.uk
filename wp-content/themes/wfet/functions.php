 ...
 *
 * Prefers a published child page of the hub with slug "guides"; otherwise CPT-only links use
 * the legacy public query var /resources/?downloads=true.
 */
add_filter(
    'query_vars',
...
function soj_resources_hub_url(): string
{
    $hub_id = soj_resources_hub_page_id();
    if ($hub_id > 0) {
    ...
    return trailingslashit(home_url('/resources'));
}

/**
 * Published page ID for the /resources/ hub (slug filterable, default resources).
 */
function soj_resources_hub_page_id(): int
{
    static $cached = null;
    if ($cached !== null) {
        return $cached;
    ...
    $slug = apply_filters('soj_resources_hub_page_slug', 'resources');
    $page = get_page_by_path($slug);
    $cached = ($page && $page->post_status === 'publish' && $page->post_type === 'page')
        ? (int) $page->ID
        : 0;
...
/**
 * Published page ID for the Guides listing (Resources CPT-only), child of the hub.
 */
function soj_resources_guides_page_id(): int
{
    static $cached = null;
    if ($cached !== null) {
        return $cached;
    ...
    $parent_id = soj_resources_hub_page_id();
    if ($parent_id < 1) {
        $cached = 0;
        return 0;
    }

    $slug = apply_filters('soj_resources_guides_page_slug', 'guides');
    $found = get_posts(
        array(
            'post_type'      => 'page',
    ...
    $cached = ! empty($found) ? (int) $found[0] : 0;

    return $cached;
}

/**
 * Permalink for the Guides page, or empty string if not published.
 */
function soj_resources_guides_url(): string
{
    $page_id = soj_resources_guides_page_id();
    if ($page_id < 1) {
        return '';
    }
...
function soj_resources_cpt_only_url(): string
{
    $guides = soj_resources_guides_url();
    if ($guides !== '') {
        return $guides;
    }

    return add_query_arg('downloads', 'true', soj_resources_hub_url());
}

/**
 * True on the Guides child page (resource CPT listing, no blog merge).
 */
function soj_is_resources_guides_listing_page(): bool
{
    if (! is_singular('page')) {
        return false;
    }

    $guides_id = soj_resources_guides_page_id();

    return $guides_id > 0 && (int) get_queried_object_id() === $guides_id;
}

/**
 * True when pagination links should append ?downloads=true (legacy hub URL only).
 */
function soj_resources_cpt_pagination_use_downloads_query(): bool
{
    return in_array(strtolower((string) get_query_var('downloads')), array('true', '1'), true);
}

function soj_is_resources_cpt_only_view(): bool
{
    if (soj_is_resources_guides_listing_page()) {
        return true;
    }
