...
define( 'ITSEC_QR_PAD0', 0xEC );
define( 'ITSEC_QR_PAD1', 0x11 );
...
class ITSEC_QRCode {
	...
	function __construct() {
		$this->typeNumber        = 1;
		$this->errorCorrectLevel = ITSEC_QR_ERROR_CORRECT_LEVEL_H;
	...
	function addData( $data, $mode = 0 ) {
		...
		switch ( $mode ) {
			case ITSEC_QR_MODE_NUMBER:
				$impl = new ITSEC_ITSEC_QRNumber( $data );
				$this->addDataImpl( $impl );
				break;
			case ITSEC_QR_MODE_ALPHA_NUM:
				$impl = new ITSEC_ITSEC_QRAlphaNum( $data );
				$this->addDataImpl( $impl );
				break;
			case ITSEC_QR_MODE_8BIT_BYTE:
				$impl = new ITSEC_ITSEC_QR8BitByte( $data );
				$this->addDataImpl( $impl );
				break;
			case ITSEC_QR_MODE_KANJI:
				$impl = new ITSEC_ITSEC_QRKanji( $data );
				$this->addDataImpl( $impl );
				break;
	...
	function getBestMaskPattern() {
		...
		for ( $i = 0; $i < 8; $i ++ ) {
			...
			$lostPoint = ITSEC_QRUtil::getLostPoint( $this );
	...
	function makeImpl( $test, $maskPattern ) {
		...
		for ( $i = 0; $i < $this->moduleCount; $i ++ ) {
			$this->modules[] = ITSEC_QRCode::createNullArray( $this->moduleCount );
		}
		...
		$data = ITSEC_QRCode::createData( $this->typeNumber, $this->errorCorrectLevel, $dataArray );
			...
			while ( true ) {
				...
				for ( $c = 0; $c < 2; $c ++ ) {
					...
					if ( $this->modules[ $row ][ $col - $c ] === null ) {
						...
						if ( ITSEC_QRUtil::getMask( $maskPattern, $row, $col - $c ) ) {
							$dark = ! $dark;
						}
	...
	function setupPositionAdjustPattern() {
		...
		$pos = ITSEC_QRUtil::getPatternPosition( $this->typeNumber );
	...
	function setupTypeNumber( $test ) {
		...
		$bits = ITSEC_QRUtil::getBCHTypeNumber( $this->typeNumber );
	...
	function setupTypeInfo( $test, $maskPattern ) {
		...
		$bits = ITSEC_QRUtil::getBCHTypeInfo( $data );
	...
	function createData( $typeNumber, $errorCorrectLevel, $dataArray ) {
		...
		$rsBlocks = ITSEC_QRRSBlock::getRSBlocks( $typeNumber, $errorCorrectLevel );

		$buffer = new ITSEC_QRBitBuffer();

		for ( $i = 0; $i < count( $dataArray ); $i ++ ) {
			/** @var \ITSEC_QRData $data */
			$data = $dataArray[ $i ];
		...
		while ( true ) {
			...
			$buffer->put( ITSEC_QR_PAD0, 8 );

			if ( $buffer->getLengthInBits() >= $totalDataCount * 8 ) {
				break;
			}
			$buffer->put( ITSEC_QR_PAD1, 8 );
		}

		return ITSEC_QRCode::createBytes( $buffer, $rsBlocks );
	}

	/**
	 * @param \ITSEC_QRBitBuffer $buffer
	 * @param \ITSEC_QRRSBlock[] $rsBlocks
	 *
	 * @return array
	 */
	function createBytes( &$buffer, &$rsBlocks ) {
		...
		$dcdata = ITSEC_QRCode::createNullArray( count( $rsBlocks ) );
		$ecdata = ITSEC_QRCode::createNullArray( count( $rsBlocks ) );
		...
		for ( $r = 0; $r < $rsBlockCount; $r ++ ) {
			...
			$dcdata[ $r ] = ITSEC_QRCode::createNullArray( $dcCount );
			...
			$rsPoly  = ITSEC_QRUtil::getErrorCorrectPolynomial( $ecCount );
			$rawPoly = new ITSEC_QRPolynomial( $dcdata[ $r ], $rsPoly->getLength() - 1 );

			$modPoly      = $rawPoly->mod( $rsPoly );
			$ecdata[ $r ] = ITSEC_QRCode::createNullArray( $rsPoly->getLength() - 1 );
		...
		$data = ITSEC_QRCode::createNullArray( $totalCodeCount );
...
define( 'ITSEC_QR_G15', ( 1 << 10 ) | ( 1 << 8 ) | ( 1 << 5 )
                        | ( 1 << 4 ) | ( 1 << 2 ) | ( 1 << 1 ) | ( 1 << 0 ) );

define( 'ITSEC_QR_G18', ( 1 << 12 ) | ( 1 << 11 ) | ( 1 << 10 )
                        | ( 1 << 9 ) | ( 1 << 8 ) | ( 1 << 5 ) | ( 1 << 2 ) | ( 1 << 0 ) );

define( 'ITSEC_QR_G15_MASK', ( 1 << 14 ) | ( 1 << 12 ) | ( 1 << 10 )
                             | ( 1 << 4 ) | ( 1 << 1 ) );
...
class ITSEC_QRUtil {
	...
	static function getMaxLength( $typeNumber, $mode, $errorCorrectLevel ) {
		...
		switch ( $errorCorrectLevel ) {
			case ITSEC_QR_ERROR_CORRECT_LEVEL_L :
				$e = 0;
				break;
			case ITSEC_QR_ERROR_CORRECT_LEVEL_M :
				$e = 1;
				break;
			case ITSEC_QR_ERROR_CORRECT_LEVEL_Q :
				$e = 2;
				break;
	...
	static function getErrorCorrectPolynomial( $errorCorrectLength ) {
		...
		$a = new ITSEC_QRPolynomial( array( 1 ) );

		for ( $i = 0; $i < $errorCorrectLength; $i ++ ) {
			$a = $a->multiply( new ITSEC_QRPolynomial( array( 1, ITSEC_QRMath::gexp( $i ) ) ) );
		}
	...
	static function getMask( $maskPattern, $i, $j ) {
		...
		switch ( $maskPattern ) {
			...
			case ITSEC_QR_MASK_PATTERN000 :
				return ( $i + $j ) % 2 == 0;
			case ITSEC_QR_MASK_PATTERN001 :
				return $i % 2 == 0;
			case ITSEC_QR_MASK_PATTERN010 :
				return $j % 3 == 0;
			case ITSEC_QR_MASK_PATTERN011 :
				return ( $i + $j ) % 3 == 0;
			case ITSEC_QR_MASK_PATTERN100 :
				return ( floor( $i / 2 ) + floor( $j / 3 ) ) % 2 == 0;
			case ITSEC_QR_MASK_PATTERN101 :
				return ( $i * $j ) % 2 + ( $i * $j ) % 3 == 0;
			case ITSEC_QR_MASK_PATTERN110 :
				return ( ( $i * $j ) % 2 + ( $i * $j ) % 3 ) % 2 == 0;
			case ITSEC_QR_MASK_PATTERN111 :
	...
	static function getMode( $s ) {
		if ( ITSEC_QRUtil::isAlphaNum( $s ) ) {
			if ( ITSEC_QRUtil::isNumber( $s ) ) {
				return ITSEC_QR_MODE_NUMBER;
			}

			return ITSEC_QR_MODE_ALPHA_NUM;
		} elseif ( ITSEC_QRUtil::isKanji( $s ) ) {
			return ITSEC_QR_MODE_KANJI;
		} else {
			return ITSEC_QR_MODE_8BIT_BYTE;
		}
	}
	...
	static function isNumber( $s ) {
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$c = ord( $s[ $i ] );
			if ( ! ( ITSEC_QRUtil::toCharCode( '0' ) <= $c && $c <= ITSEC_QRUtil::toCharCode( '9' ) ) ) {
				return false;
			}
		}
	...
	static function isAlphaNum( $s ) {
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$c = ord( $s[ $i ] );
			if ( ! ( ITSEC_QRUtil::toCharCode( '0' ) <= $c && $c <= ITSEC_QRUtil::toCharCode( '9' ) )
			     && ! ( ITSEC_QRUtil::toCharCode( 'A' ) <= $c && $c <= ITSEC_QRUtil::toCharCode( 'Z' ) )
			     && strpos( " $%*+-./:", $s[ $i ] ) === false ) {
				return false;
			}
		}
	...
	static function getBCHTypeInfo( $data ) {
		$d = $data << 10;
		while ( ITSEC_QRUtil::getBCHDigit( $d ) - ITSEC_QRUtil::getBCHDigit( ITSEC_QR_G15 ) >= 0 ) {
			$d ^= ( ITSEC_QR_G15 << ( ITSEC_QRUtil::getBCHDigit( $d ) - ITSEC_QRUtil::getBCHDigit( ITSEC_QR_G15 ) ) );
		}

		return ( ( $data << 10 ) | $d ) ^ ITSEC_QR_G15_MASK;
	}

	static function getBCHTypeNumber( $data ) {
		$d = $data << 12;
		while ( ITSEC_QRUtil::getBCHDigit( $d ) - ITSEC_QRUtil::getBCHDigit( ITSEC_QR_G18 ) >= 0 ) {
			$d ^= ( ITSEC_QR_G18 << ( ITSEC_QRUtil::getBCHDigit( $d ) - ITSEC_QRUtil::getBCHDigit( ITSEC_QR_G18 ) ) );
		}
...
class ITSEC_QRRSBlock {
	...
	static function getRSBlocks( $typeNumber, $errorCorrectLevel ) {
		...
		$rsBlock = ITSEC_QRRSBlock::getRsBlockTable( $typeNumber, $errorCorrectLevel );
		...
		for ( $i = 0; $i < $length; $i ++ ) {
			...
			for ( $j = 0; $j < $count; $j ++ ) {
				$list[] = new ITSEC_QRRSBlock( $totalCount, $dataCount );
			}
		}
...
class ITSEC_ITSEC_QRNumber extends ITSEC_QRData {

	function __construct( $data ) {
		parent::__construct( ITSEC_QR_MODE_NUMBER, $data );
	}
	...
	function write( &$buffer ) {
		...
		while ( $i + 2 < strlen( $data ) ) {
			$num = ITSEC_ITSEC_QRNumber::parseInt( substr( $data, $i, 3 ) );
			$buffer->put( $num, 10 );
			$i += 3;
		}
		...
		if ( $i < strlen( $data ) ) {
			...
			if ( strlen( $data ) - $i == 1 ) {
				$num = ITSEC_ITSEC_QRNumber::parseInt( substr( $data, $i, $i + 1 ) );
				$buffer->put( $num, 4 );
			} elseif ( strlen( $data ) - $i == 2 ) {
				$num = ITSEC_ITSEC_QRNumber::parseInt( substr( $data, $i, $i + 2 ) );
				$buffer->put( $num, 7 );
			}
		}
	}
	...
	static function parseInt( $s ) {
		...
		$num = 0;
		for ( $i = 0; $i < strlen( $s ); $i ++ ) {
			$num = $num * 10 + ITSEC_ITSEC_QRNumber::parseIntAt( ord( $s[ $i ] ) );
		}
...
class ITSEC_ITSEC_QRKanji extends ITSEC_QRData {

	function __construct( $data ) {
		parent::__construct( ITSEC_QR_MODE_KANJI, $data );
	}
...
class ITSEC_ITSEC_QRAlphaNum extends ITSEC_QRData {

	function __construct( $data ) {
		parent::__construct( ITSEC_QR_MODE_ALPHA_NUM, $data );
	}
	...
	function write( &$buffer ) {
		...
		while ( $i + 1 < strlen( $c ) ) {
			$buffer->put( ITSEC_ITSEC_QRAlphaNum::getCode( ord( $c[ $i ] ) ) * 45
			              + ITSEC_ITSEC_QRAlphaNum::getCode( ord( $c[ $i + 1 ] ) ), 11 );
			$i += 2;
		}

		if ( $i < strlen( $c ) ) {
			$buffer->put( ITSEC_ITSEC_QRAlphaNum::getCode( ord( $c[ $i ] ) ), 6 );
		}
	}

	static function getCode( $c ) {

		if ( ITSEC_QRUtil::toCharCode( '0' ) <= $c
		     && $c <= ITSEC_QRUtil::toCharCode( '9' ) ) {
			return $c - ITSEC_QRUtil::toCharCode( '0' );
		} elseif ( ITSEC_QRUtil::toCharCode( 'A' ) <= $c
		           && $c <= ITSEC_QRUtil::toCharCode( 'Z' ) ) {
			return $c - ITSEC_QRUtil::toCharCode( 'A' ) + 10;
		} else {
			switch ( $c ) {
...
class ITSEC_ITSEC_QR8BitByte extends ITSEC_QRData {

	function __construct( $data ) {
		parent::__construct( ITSEC_QR_MODE_8BIT_BYTE, $data );
	}
...
class ITSEC_QRMath {
	...
	static function init() {
		...
		self::$QR_MATH_EXP_TABLE = ITSEC_QRMath::createNumArray( 256 );
		...
		self::$QR_MATH_LOG_TABLE = ITSEC_QRMath::createNumArray( 256 );
...
class ITSEC_QRPolynomial {
	...
	function __construct( $num, $shift = 0 ) {
		...
		$this->num = ITSEC_QRMath::createNumArray( count( $num ) - $offset + $shift );
	...
	function toLogString() {
		...
		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			...
			$buffer .= ITSEC_QRMath::glog( $this->get( $i ) );
		}
	...
	/**
	 * @param \ITSEC_QRPolynomial $e
	 *
	 * @return \ITSEC_QRPolynomial
	 */
	function multiply( $e ) {

		$num = ITSEC_QRMath::createNumArray( $this->getLength() + $e->getLength() - 1 );

		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			$vi = ITSEC_QRMath::glog( $this->get( $i ) );

			for ( $j = 0; $j < $e->getLength(); $j ++ ) {
				$num[ $i + $j ] ^= ITSEC_QRMath::gexp( $vi + ITSEC_QRMath::glog( $e->get( $j ) ) );
			}
		}

		return new ITSEC_QRPolynomial( $num );
	}

	/**
	 * @param \ITSEC_QRPolynomial $e
	 *
	 * @return $this|\ITSEC_QRPolynomial
	 */
	function mod( $e ) {
		...
		$ratio = ITSEC_QRMath::glog( $this->get( 0 ) ) - ITSEC_QRMath::glog( $e->get( 0 ) );

		$num = ITSEC_QRMath::createNumArray( $this->getLength() );
		for ( $i = 0; $i < $this->getLength(); $i ++ ) {
			$num[ $i ] = $this->get( $i );
		}

		for ( $i = 0; $i < $e->getLength(); $i ++ ) {
			$num[ $i ] ^= ITSEC_QRMath::gexp( ITSEC_QRMath::glog( $e->get( $i ) ) + $ratio );
		}

		$newPolynomial = new ITSEC_QRPolynomial( $num );
...
define( 'ITSEC_QR_MODE_NUMBER', 1 << 0 );
define( 'ITSEC_QR_MODE_ALPHA_NUM', 1 << 1 );
define( 'ITSEC_QR_MODE_8BIT_BYTE', 1 << 2 );
define( 'ITSEC_QR_MODE_KANJI', 1 << 3 );

//---------------------------------------------------------------
// MaskPattern
//---------------------------------------------------------------

define( 'ITSEC_QR_MASK_PATTERN000', 0 );
define( 'ITSEC_QR_MASK_PATTERN001', 1 );
define( 'ITSEC_QR_MASK_PATTERN010', 2 );
define( 'ITSEC_QR_MASK_PATTERN011', 3 );
define( 'ITSEC_QR_MASK_PATTERN100', 4 );
define( 'ITSEC_QR_MASK_PATTERN101', 5 );
define( 'ITSEC_QR_MASK_PATTERN110', 6 );
define( 'ITSEC_QR_MASK_PATTERN111', 7 );

//---------------------------------------------------------------
// ErrorCorrectLevel

// 7%.
define( 'ITSEC_QR_ERROR_CORRECT_LEVEL_L', 1 );
// 15%.
define( 'ITSEC_QR_ERROR_CORRECT_LEVEL_M', 0 );
// 25%.
define( 'ITSEC_QR_ERROR_CORRECT_LEVEL_Q', 3 );
// 30%.
define( 'ITSEC_QR_ERROR_CORRECT_LEVEL_H', 2 );
