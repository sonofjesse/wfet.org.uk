...
- Improvement: Metadata sent to the Cloud API

= 5.4.4 – 24 April, 2026 =

- Improvement: Only recreate Cloudflare rules when changes are detected during plugin updates
- Improvement: Include WooCommerce auto-purged URLs in the `flying_press_auto_purge_urls` filter
- Improvement: Metadata sent to the Cloud API

= 5.4.3 – 18 April, 2026 =

- Improvement: Updated internal licensing SDK

= 5.4.2 – 18 April, 2026 =

- Improvement: Omit sensitive keys when sending data to cloud APIs

= 5.4.1 – 09 April, 2026 =

- Improvement: Better language support especially for Italian and Turkish languages
- Fix: An error while setting the heartbeat frequency in PHP versions below 8.1

= 5.4.0 – 07 April, 2026 =

- New: Added language support for German, Spanish, French, Italian, Portuguese (Brazil), Polish, Korean, Turkish, Dutch, Czech, Arabic, Japanese, and Chinese (Traditional)
- Fix: Changed the heartbeat filter to control the heartbeat frequency

= 5.3.5 – 03 April, 2026 =

- Improvement: Upgrade desktop and mobile user agents for preloading
- Improvement: Include site language for Cloud Optimizer

= 5.3.4 - 26 March, 2026 =

- Improvement: Prevent already optimized images from being added to the optimization queue
- Improvement: Skip excluded URLs when adding to the preload queue
- Improvement: New fallback (htaccess/nginx) rules to serve original images when the browser does not support WebP/AVIF
- Improvement: Core Web Vitals Tracking is now enabled by default for new installations

= 5.3.3 - 03 March, 2026 =

- Improvement: Show image optimization stats for individual images in the Media Library
- Fix: Cloudflare page caching was getting disabled in certain cases
- Fix: "Delete Original Images" was not deleting an original image of scaled images
- Fix: Cloudflare page cache was not getting cleared with auto-purge in certain cases
- Fix: A warning of `Passing null to parameter` from Image Optimizer
- Fix: A warning of `Undefined array key "file"` from Page Optimizer

= 5.3.2 – 06 February, 2026 =

- Improvement: Exclude specific images by name or keyword from being optimized
- Improvement: Lowered the auto-purge priority for ACF and WooCommerce integrations
- Improvement: Prevented purging and preloading all pages when an image is auto-optimized
- Improvement: Prevented the Image Optimizer stats API from being cached by switching it to the POST method
- Fix: Undefined array key 'filesize' in the Image Optimizer status API response
- Fix: Prevented folder URLs from being sent to the Image Optimizer
- Fix: Responsive plugin UI for better user experience

= 5.3.1 – 20 January, 2026 =

- Improvement: **Image Optimization** - Added a Stop Optimization button to stop ongoing image optimization

= 5.3.0 – 20 January, 2026 =

- New: **Image Optimization** — compress JPEG/JPG & PNG images and convert them to WEBP or AVIF to reduce image size and speed up your site

= 5.2.5 – 08 January, 2026 =

- Improvement: Lower the priority of FlyingPress actions to ensure WooCommerce actions are processed first
- Improvement: Increase Action Scheduler batch size to 100 to speed up cache preloading
- Fix: Cloudflare integration failing with API errors in certain cases

= 5.2.4 - 23 December, 2025 =

- Improvement: Cloudflare integration just got better with more reliable cache handling
- Improvement: Add Super Page Cache to the incompatible plugins list
- Improvement: Prevent cache folders from being created with uppercase letters
- Fix: Incorrect rules got removed in the Cloudflare integration when FlyingPress was deactivated
- Fix: Ignore query parameters were getting added to the preload queue in certain cases
- Fix: Upgrade tasks were skipped in certain cases when FlyingPress was updated
- Fix: Error while upgrading the official Cloudflare plugin to 4.14.0 or higher

= 5.2.3 - 17 November, 2025 =

- Improvement: Cloud-based optimization got better with more speed and accuracy
- Improvement: Concurrency in Action Scheduler is no longer forced to 1 and now respects the global concurrency
- Fix: In the Cloudflare integration bypass cookies were not properly synced to Cloudflare when modified 

= 5.2.2 - 07 October, 2025 =

- Improvement: Properly rewrite `.htaccess` file with locking mechanism and only when necessary 

= 5.2.1 - 06 October, 2025 =

- Improvement: Significantly reduced high CPU usage while preloading cache
- Fix: Properly cleanup `.htaccess` and `advanced-cache.php` contents after deactivation, when Cloudflare integration was enabled
- Fix: An issue where delay third-party/selected scripts was not working correctly when using "Delay all JavaScript" with the "Load after interaction" mode
- Fix: A warning of undefined key `cache_mobile` in Cloudflare integration
- Fix: Exclude Cloudflare credentials from exported configuration

= 5.2.0 - 25 September, 2025 =

New: **Real-User Core Web Vitals Tracking** — measure real performance, fix what's slowing visitors!

= 5.1.2 - 25 August, 2025 =

- Improvement: Action Scheduler as the new background processing library for broader compatibility
- Improvement: Cache static assets separately in Cloudflare integration
- Improvement: Used CSS generation is faster than before
- Improvement: Upgrade some dependencies for enhanced performance
- Improvement: Default delay between each preload is 0.5s 
- Improvement: Default timeout for loading scripts is now 10s
- Removed: Disable WP Cron is permanently removed from the bloat settings
- Fix: Critical images were incorrectly detected in certain cases

= 5.1.1 - 18 August, 2025 =

- Fix: An error while configuring Cloudflare integration on PHP versions below 8.1

= 5.1.0 - 12 August, 2025 =

- New: **Cloudflare Integration** for full page caching and faster global TTFB
- Improvement: Prevent writing the constant to `wp-config.php` file when `DISABLE_WP_CRON` is manually added
- Fix: Cache preloading was not started in certain cases

= 5.0.7 - 17 July, 2025 =

- Improvement: Atomic write to `wp-config.php` file for broader compatibility
- Fix: Import configurations were not working in certain cases

= 5.0.6 - 11 July, 2025 =

- Improvement: Lifecycle events from delayed scripts now fire reliably after loading ensuring no duplicate trigger


= 5.0.5 - 03 July, 2025 =

- Improvement: Add `typekit.net` to default self-host domain list
- Improvement: Set fallback type to `woff2` while preload fonts if font type is not available
- Fix: A JS error while lazy loading iframes and videos
