...
class Caching
{
  ...
  public static function is_url_cacheable($url)
  {
    ...
    // Check if the URL contains any unknown query parameters
    foreach ($query_args as $key => $value) {
      if (!in_array($key, $known_queries, true)) {
        return false;
      }
    }
    ...
    if (preg_match($regex, $full_url)) {
      return false;
    }

    // Check if current page is on the excluded pages list
    if (Utils::any_keywords_match_string($config['cache_bypass_urls'], $full_url)) {
      return false;
    }
